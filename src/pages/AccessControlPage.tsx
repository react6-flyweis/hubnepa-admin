...
export default function AccessControlPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-4">
      ...
      >
        <TabsList className="grid h-8 w-full grid-cols-3 rounded-xl border border-slate-200 bg-white p-1">
          {tabItems.map((tab) => (
            <TabsTrigger
              key={tab.value}
              value={tab.value}
              className="h-6 rounded-lg text-sm text-slate-500 data-active:bg-slate-100 data-active:text-slate-900"
            >
              {tab.label}
            </TabsTrigger>
          ))}
        ...
        <TabsContent value="overview" className="mt-4 space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            ...
            <Card className="flex min-h-64 items-center justify-center rounded-2xl border border-slate-300 bg-slate-50 p-6 shadow-none">
        ...
        <TabsContent value="admins" className="mt-4">
          <Card className="rounded-2xl border border-slate-200 bg-white p-6 text-sm text-slate-500 shadow-none">
            Admin user assignments will appear here.
          </Card>
        </TabsContent>

        <TabsContent value="audit" className="mt-4">
          <Card className="rounded-2xl border border-slate-200 bg-white p-6 text-sm text-slate-500 shadow-none">
            Audit log records will appear here.
          </Card>
        </TabsContent>
      </Tabs>
    </div>
